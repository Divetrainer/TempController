...
sensor_screen_i2c = I2C(1, scl=scl_p11, sda=sda_p10, freq=400000)


sensor_oled = SSD1306_I2C(128, 32, sensor_screen_i2c, addr=0x3C)
...
while (True):
    ...
    for rom in roms:
        ...
        if temp_f != sensor_recorded_temp:
            sensor_oled.fill(0)
            sensor_oled.text('Sensor temp:', 0,0)
            sensor_oled.text("{:.2f}".format(temp_f),0,20)
            sensor_oled.show()
            print()
